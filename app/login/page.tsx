...
export default async function LoginPage({
  ...
  searchParams: Promise<{ error?: string; returnTo?: string; stage?: string; reason?: string }>;
}) {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#20211f] px-5 py-12">
      <section className="w-full max-w-md rounded-[28px] bg-[#f7f7f4] p-7 shadow-2xl md:p-9">
        ...
        {query.error && <div role="alert" className="mb-5 rounded-xl bg-red-50 p-3 text-sm text-red-700">{query.error === "locked" ? "Too many attempts. Please wait 15 minutes." : query.error === "storage" ? `The private library could not be opened. Reference: ${query.stage ?? "unknown"}.` : `The email or password is incorrect. Reference: ${query.reason ?? "credentials"}.`}</div>}
